...
class RoleLoginWidget(QWidget):
    ...
    def _build(self):
        ...
        card = QFrame(); card.setFixedWidth(380)
        ...
        """)
        cl = QVBoxLayout(card); cl.setContentsMargins(36, 32, 36, 30); cl.setSpacing(12)

        # Avatar + title + subtitle
        av = QLabel(self.icon); av.setFixedSize(54, 54)
        ...
        av.setStyleSheet(
            ...
            f"border-radius:6px;font-size:22px;")
        av_row = QHBoxLayout(); av_row.addStretch(); av_row.addWidget(av); av_row.addStretch()
        cl.addLayout(av_row)

        title = mklbl(f"{self.label} Login", L_TEXT, 19, bold=True)
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(title)

        sub = mklbl(_SUBTITLE[self.role], L_DIM, 12)
        ...
        # ── Identify yourself ────────────────────────────────────────────
        cl.addWidget(mklbl(f"{self.label} ID", L_DIM, 10.5))
        self.id_edit = _field(f"e.g. {'DR-001' if self.role=='doctor' else 'PT-001'}")
        cl.addWidget(self.id_edit)

        cl.addWidget(mklbl("Registered Email", L_DIM, 10.5))
        self.email_edit = _field("your@email.com")
        cl.addWidget(self.email_edit)

        self.send_otp_btn = mkbtn("Send Login OTP", ACCENT, wide=True)
        ...
        self.otp_edit = _field("8-character OTP", mono=True)
        self.otp_edit.setMaxLength(8)
        ob.addWidget(self.otp_edit)

        self.verify_btn = mkbtn(f"Login as {self.label}", ACCENT, wide=True)
